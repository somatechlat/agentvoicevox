...
class TenantSettingsService:
    ...
    def get_settings(tenant_id: UUID) -> TenantSettings:
        ...
        tenant = TenantService.get_by_id(tenant_id)
        try:
            return tenant.extended_settings
        except TenantSettings.DoesNotExist:
    ...
    def update_settings(
        ...
        **kwargs,
        ...
        Raises:
            NotFoundError: If tenant not found
        """
        settings = TenantService.get_settings(tenant_id)
    ...
    @staticmethod
    def check_limit(tenant: Tenant, resource: str, current_count: int) -> bool:
        """
        Check if tenant has reached a resource limit.
        
        Args:
            tenant: The tenant to check
            resource: Resource type (users, projects, api_keys, sessions)
    ...
    @staticmethod
    def enforce_limit(tenant: Tenant, resource: str, current_count: int) -> None:
        """
        Enforce tenant resource limit.
        
        Raises:
            TenantLimitExceededError: If limit exceeded
        """
        if not TenantService.check_limit(tenant, resource, current_count):
            raise TenantLimitExceededError(
                f"Tenant has reached the maximum number of {resource}. "
                f"Please upgrade your plan."
            )
    
    @staticmethod
    def get_usage_stats(tenant_id: UUID) -> Dict[str, Any]:
        """
        ...
        return {
            "users": {
                "current": User.all_objects.filter(tenant=tenant, is_active=True).count(),
                "limit": tenant.max_users,
            },
            "projects": {
                "current": Project.all_objects.filter(tenant=tenant).count(),
                "limit": tenant.max_projects,
            },
            "api_keys": {
                "current": APIKey.all_objects.filter(tenant=tenant, revoked_at__isnull=True).count(),
                "limit": tenant.max_api_keys,
            },
            "sessions_this_month": {
                "current": Session.all_objects.filter(
                    tenant=tenant,
